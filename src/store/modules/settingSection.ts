...
export const settingSectionModule = defineStore(ModuleName.SETTINGS_SECTION, {
	...
	actions: {
		...
		set_current_section (value: TSettingSection|undefined) {
			...
			if (value && !pathIncludesSection) router?.replace({ path: route?.value.path, query: { section: value } });
			else if (this.current_section && !pathIncludesSection) router?.replace({ path: route?.value.path, query: { section: value } });
