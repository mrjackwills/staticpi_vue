...
const pauseDevice_confirm = async (authentication?: TAuthObject): Promise<void> => {
	loading.value = true;
	const response = await axios_device.paused_patch({ pause: !paused.value, name: name_of_device.value, authentication });
	loading.value = false;
	if (response) {
		snackSuccess({ message: `${paused.value ? 'resumed':'paused'} device : ${name_of_device.value}`, icon: paused.value ? mdiPlayCircle: mdiPauseCircle });
		emit('refresh');
	}
};
