			...
			<section >
				...
				<v-expand-transition>
					<v-row v-if='!showAdd' align='center' justify='center' no-gutters class='ma-0 pa-0 mb-3'>
						...
						<v-col cols='12' md='auto' :class='smAndDown? `mt-3`:`mr-3`' v-if='devices.length >0 && !showAdd' :order='smAndDown? "2" : "1"' class='ma-0 pa-0 ' >
			...
			<section v-if='showAdd' >
				<v-expand-transition>
					<AddDevice
						v-if='showAdd'
						@refresh='getData'
						@show-add-new-device='showAddNewDevice'
						class=''
					/>
				</v-expand-transition>
			</section>
...
const disabled = computed((): boolean => {
	return devices.value.length >= userStore.maxDevices;
});
