import type { HttpCode } from '@/types/enum_http';
...
export type TSnackPosition = {x: TPositionX; y: TPositionY }
...
export type TRootState = {user: { _email:string, _authenticated: boolean, _maxBandwidth: string, _maxClients: number, _maxMessageSize: number, _maxDevices: number, _userLevel: UserLevel } }
...
export type TDeviceInfo = { max_clients: number}
	& { [K in 'name_of_device' |'api_key'| 'creation_date'] : string}
	& { [ K in 'client_password_required' | 'device_password_required' | 'paused' | 'structured_data' ]: boolean}
	& { [ K in 'ip' | 'timestamp_online' | 'timestamp_offline' ]: string | undefined}
	& { [K in TBandwidthInOut] : string}

export type TLimits = { name_of_device: string, ttl: number}
	
export type TShowMoreInfo = { name_of_device: string, value: boolean}

export type TSortableColumns = 'connection' | 'name' | 'bandwidth' | 'max clients' |'structured data' |'device password' | 'status'
export type TSortedBy = { name?: TSortableColumns, largestFirst: boolean }
...
export type TFooterRow = { [K in 'text' | 'tooltip']: string } & { model: TFooterModel }
...
export type TExtraBandwidthDetailed = { period:	'last 24 hours'|'this month'|'all time'}
	& { [k in TExtraTime] : {in: TBandwidth, out: TBandwidth, total: TBandwidth}}
...
type BaseTextFields = { [K in 'appendIcon'| 'autocomplete' | 'icon'| 'label' |'type']: string}

export type TRegisterModels = BaseFields | 'full_name'| 'invite'

export type TWsAuth = { [K in 'key'| 'password']: string}

export type TRegisterTextField = BaseTextFields & { clearable: boolean, model: TRegisterModels }
...
export type TGlobalBandwidth = { [ K in TGlobalInOut]: number}

export type TRegisterUser = { [ K in 'email' | 'full_name' | 'invite' | 'password'] : string } & { [ K in 'agree'|'age']: boolean }
export type TPasswordPatch = {[K in 'resetId' | 'password']: string } & {token:string|undefined}
export type TSignin = TAuthObject & { email: string, remember: boolean }
export type TPasswordChange = {[K in 'current_password' | 'new_password']: string} & {token?: string}
...
export type TResetPasswordGet = { [ K in 'two_fa_enabled'|'two_fa_backup']: boolean}
...
export type TDeviceNamedPatch = { patch: DevicePatch, name: string, auth?: TAuthObject}
...
export type TDevicePasswordPatch = TBaseDevicePatch & { [ K in 'device_password'|'client_password' ]:string}
...
type TChangePasswordBase = { [ K in 'autocomplete' | 'icon' | 'label' | 'type' | 'appendIcon']: string }
...
export type TUpdate = { [ k in 'message' | 'server' | 'time'] : string} & { status:boolean }

export type TServerName = 'website' | 'api' | 'wss' | 'token'

export type TOnlineResponse = {[k in 'api_version' | 'uptime' ]:string }
export type TServers = { [k in 'address' | 'updateTime']: string } & { [k in 'status' | 'loading']:boolean } & {description: TServerName} & TOnlineResponse

export type TComputedBandwidth = { [ K in 'icon' | 'timespan'] : string } & { [ K in 'in'| 'out']: { human_readable: TConvertBytes, bytes: number}}

export type TNavigationLink = { [ K in 'icon' | 'message' | 'route']: string }

export type TStructuredKey = 'unique' | 'error' | 'cache' | 'data'

export type TAddressRow = { [ K in 'name' | 'address' | 'toCopy' | 'tooltipMessage' | 'hoverMessage' ] : string }

// Admin
export type TAdminMemory = { [ K in 'uptime' | 'uptime_app' | 'virt' | 'rss']: number};

// block?
export type TAdminLimit = { [ K in 'points'|'max'|'ttl']: number } & { key: string, blocked: boolean }

export type TAdminUser =
	{ [ k in 'full_name' | 'email' | 'timestamp' | 'user_level' | 'registered_user_id']: string } &
	{ [ k in 'login_attempt_number' | 'two_fa_backup_count' | 'device_count']: number } &
	{ [ k in 'password_reset' | 'two_fa_enabled' | 'active']: boolean } & { [K in TBandwidthInOut] : string}

export type TAdminSession = {'key': string} &
	{ [ k in 'ttl' | 'timestamp']: number }
...
export type AdminConnection = { [ k in 'timestamp' | 'device_id']: number } &
		{ [ k in 'device_type' | 'ulid' | 'ip']: string }
...
export type TAdminEmailCount = { [ k in 'hour' | 'total']: number }
export type TAdminConnectedCount = { [ k in 'pi' | 'client']: number }

export type TAdminContactMessage = { contact_message_id :number, registered_user_id: number|null} & { [K in 'ip'|'timestamp'|'message'|'user_agent' |'email']: string};

export type TAdminConnectionRemove = { [ K in 'connection_ulid'|'device_type']: string} & {device_id: number}

export type TAdminEmailDevice = TAdminUserDelete & { device_name: string }
export type TAdminUserDelete = { email: string } & TAuthObject
export type TAdminInvite = { count: number, invite: string }
export type TAdminInvitePost = TAdminInvite & TAuthObject
export type TContact = { [ k in 'email' | 'message']: string }
